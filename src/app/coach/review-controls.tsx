...
import { saveDayCheck, saveDayComment, saveWeekComment, saveWeekReviewed } from "./actions";
...
/**
 * One day's red pen. The ✓ is shared (either coach checking checks it for
 * the team); the comment is the signed-in coach's OWN (0009) — the other
 * coach's day comment shows read-only above, attributed, so nobody steps on
 * anybody's toes. Saves on the button/blur, not on every keystroke.
 */
export function DayReviewControls({
  ...
  /** The signed-in coach's own saved comment on this day. */
  initialComment: string | null;
  /** Other coaches' comments on this day, read-only. */
  ...
  // The ✓ saves on its own, optimistically — revert on failure so a green
  // check the database doesn't have never lingers.
  function toggleCheck() {
    const next = !checked;
    setChecked(next);
    setError(null);
    startTransition(async () => {
      let res: Awaited<ReturnType<typeof saveDayCheck>>;
      try {
        res = await saveDayCheck(athleteId, dateISO, next);
      } catch {
  ...
  function saveComment() {
    setError(null);
    startTransition(async () => {
      let res: Awaited<ReturnType<typeof saveDayComment>>;
      try {
        res = await saveDayComment(athleteId, dateISO, comment);
      } catch {
        res = { ok: false, error: "Couldn't reach the server — check your connection and try again." };
      }
      if (!res.ok) {
  ...
  return (
    <div className="mt-3 border-t border-line pt-3">
      {peerComments.map((p, i) => (
        <p key={i} className="mb-2 text-[13px] leading-snug text-ink">
          <span className="font-bold text-navy">{p.coachName}:</span> {p.comment}
        </p>
      ))}
      <div className="flex flex-wrap items-start gap-2">
        ...
        <div className="min-w-[200px] flex-1">
          <input
            ...
            maxLength={2000}
            placeholder="Your comment for this day…"
            className={INPUT}
            onKeyDown={(e) => {
              if (e.key === "Enter" && dirty) saveComment();
            }}
